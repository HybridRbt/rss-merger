...
class DateFilteredRSSAggregator:
    def __init__(self, output_file=\'final_rss.xml\', html_file=\'final_daily_news.html\'):
        ...
        # 获取当前CST时间
        current_cst = datetime.now(self.cst_timezone)
        
        # 判断是否在CST午夜0点到1点之间运行
        # 如果是，则筛选前一天的文章
        if current_cst.hour == 0:
            self.today_cst = current_cst - timedelta(days=1)
        else:
            self.today_cst = current_cst
            
        self.today_date_str = self.today_cst.strftime(\'%Y-%m-%d\')
        
        print(f"当前CST时间: {current_cst}")
        print(f"筛选日期: {self.today_date_str}")

    def _is_today_cst(self, pub_date):
        """检查文章发布日期是否为筛选日期（CST时间）"""
        try:
